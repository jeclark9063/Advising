$(function() {
  ...
  function updateUserEvents(callback) {
    getAccessToken(function(accessToken) {
      if (accessToken) {
        // Create a Graph client
        var client = MicrosoftGraph.Client.init({
        ...
        client
          .api('/me/events/AAMkAGY2NzVlZTA3LTdlNTktNDRhMS1hZDQ2LTAyMjczNmRhMDY1YwBGAAAAAACXvplRls1bRqE8Yzkh471dBwBd0mJFs0T2QqMocEhCSnkHAAAAAA5NAABZNXeyyHoeS6prYfCnaMhQAARaJ_WwAAA=')
          .patch(test, (err, res) => {
            if (err) {
              console.log(err);
              return;
...
/*
  ...
  $('#test-btn').on('click', function (e) {
    //test update events
    updateUserEvents(function(events, error) {
      if (error) {
        renderError('getUserEvents failed', error);
      } else {
        console.log("twerked");
